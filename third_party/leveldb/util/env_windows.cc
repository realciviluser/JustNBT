...
// JustNBT: file names are UTF-8; use the wide Win32 API so non-ASCII paths work.
...
std::string Utf8(const wchar_t* wide) {
  const int n = ::WideCharToMultiByte(CP_UTF8, 0, wide, -1, nullptr, 0, nullptr,
                                      nullptr);
 ...
 public:
  ...
  Status NewSequentialFile(const std::string& filename,
    ...
    *result = nullptr;
    ...
    ScopedHandle handle = ::CreateFileW(
        Wide(filename).c_str(), desired_access, share_mode,
  ...
  Status NewRandomAccessFile(const std::string& filename,
    ...
    *result = nullptr;
    ...
    DWORD share_mode = FILE_SHARE_READ;
    ScopedHandle handle =
        ::CreateFileW(Wide(filename).c_str(), desired_access, share_mode,
  ...
  bool FileExists(const std::string& filename) override {
    return GetFileAttributesW(Wide(filename).c_str()) != INVALID_FILE_ATTRIBUTES;
  }

  Status GetChildren(const std::string& directory_path,
                     std::vector<std::string>* result) override {
    const std::string find_pattern = directory_path + "\\*";
    WIN32_FIND_DATAW find_data;  // JustNBT: wide API
    HANDLE dir_handle = ::FindFirstFileW(Wide(find_pattern).c_str(), &find_data);
    ...
    do {
      result->emplace_back(Utf8(find_data.cFileName));
  ...
  Status RemoveFile(const std::string& filename) override {
    if (!::DeleteFileW(Wide(filename).c_str())) {
      return WindowsError(filename, ::GetLastError());
    }
    return Status::OK();
  }
  ...
  Status CreateDir(const std::string& dirname) override {
    if (!::CreateDirectoryW(Wide(dirname).c_str(), nullptr)) {
  ...
  Status RemoveDir(const std::string& dirname) override {
    if (!::RemoveDirectoryW(Wide(dirname).c_str())) {
      return WindowsError(dirname, ::GetLastError());
    }
    return Status::OK();
  }
  ...
  Status GetFileSize(const std::string& filename, uint64_t* size) override {
    ...
    if (!::GetFileAttributesExW(Wide(filename).c_str(), GetFileExInfoStandard,
                                &file_attributes)) {
      return WindowsError(filename, ::GetLastError());
    }
  ...
  Status RenameFile(const std::string& from, const std::string& to) override {
    ...
    // exist.
    if (::MoveFileW(Wide(from).c_str(), Wide(to).c_str())) {
      return Status::OK();
    }
    ...
    if (::ReplaceFileW(Wide(to).c_str(), Wide(from).c_str(), /*lpBackupFileName=*/nullptr,
                       REPLACEFILE_IGNORE_MERGE_ERRORS,
  ...
  Status LockFile(const std::string& filename, FileLock** lock) override {
    *lock = nullptr;
    Status result;
    ScopedHandle handle = ::CreateFileW(
        Wide(filename).c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ,
        /*lpSecurityAttributes=*/nullptr, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL,
        nullptr);
    if (!handle.is_valid()) {
  ...
  Status NewLogger(const std::string& filename, Logger** result) override {
    std::FILE* fp = ::_wfopen(Wide(filename).c_str(), L"w");  // JustNBT
    if (fp == nullptr) {
      *result = nullptr;
      return WindowsError(filename, ::GetLastError());
    } else {
